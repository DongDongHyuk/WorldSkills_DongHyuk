...
# A : br,it,turn,spin_count,pack,move_dire
# B : br,pack_dire,move_dire
def bfs_sorting(root:'State_0 or State_2'):
    def get_br(n:'state'):
        if not G_Type:
            return n.br + str(n.it) + str(n.turn) + str(n.pack)
        else:
            return str(n.br) + str(n.pack_dire) + str(n.move_dire)
    que = deque()
    que.append(root)
    marked = {root:'root'}
    br_marked = {get_br(root):'root'}
    state = root
    def isleaf(br):
        if not G_Type:
...
n = [[0, '4103x000x00000x0x0000324', '30100103', 2, 20],
    [1, ['1000x000000x', '0000000xx100'], [3, 3], [0, 9], [10, 10]]]
