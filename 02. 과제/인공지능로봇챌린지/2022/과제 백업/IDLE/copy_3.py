...
# A board return value : now,new,turn,spin_count
# B board return value : now,new,pack_dire,move_dire
def convert(result):
    path = []
    first = result[0]
    for second in result[1:]:
        step = ([[None]*2,[None]*2,second.pack_dire,second.move_dire] if G_Type else 
                [None,None,second.turn,second.spin_count])
        for i in range(2):       
...
# - A : Tpye, br, index table, start, end
# - B : Tpye, br, pack dire, start, end
def main(n):
