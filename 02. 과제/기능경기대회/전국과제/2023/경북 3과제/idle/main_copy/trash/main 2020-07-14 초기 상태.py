...
def sort(m,leaf,e,p=-1):
    ...
    res = []
    s = m.index(pack)
    if t is 2 and s is not e and len(exp(0,m,e)) is 1:
        m = bfs(1,m,leaf,e,'0')
    r = bfs(0,m,s,e)
    for i in r:
...
def main(g_t,m,*a):    
    ...
    if t == 2:
        ...
        if pos:         # 중앙에 고정팩 있을때
            ...
            '''
            윤곽을 먼저 정렬하고 회전초밥으로 부셈
            '''
##            printf(''.join(['H' if i in hli else ' ' for i in range(16)]),4,4)
##            printf(leaf,4,4)
                        
            li = {5:[3,7,15,14,11,12,13], 6:[0,4,12,13,8,15,14],
...
##    Type,root,leaf,a = 2,'0034x0x06108052x','0560x0x41203800x',[0, 5]
