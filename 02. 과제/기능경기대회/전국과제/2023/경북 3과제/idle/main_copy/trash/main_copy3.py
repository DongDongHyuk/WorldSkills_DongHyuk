...
def exp(n,m,pos=-1,pack=-1):
    ...
    for i in range(size) if n > 0 else [pos]:       
        ...
        if t == 2 and n > 0 and i in hli:        # 홀 위에는 팩을 둘 수 없음
            continue        
        for j in aro(i):
            b1 = lambda i: m[i] not in (['0x','x'][t == 2] if n > 0 else 'x') and i not in fix
...
def main(g_t,m,*a):    
    ...
    if t == 2:
        ...
        if pos:
            pos = pos[0]        # 중앙 고정팩 위치                    
            outli = {5:[3,12,7,13,15,14,11], 6:[0,15,4,14,12,13,8],
                  9:[0,15,1,11,3,2,7], 10:[3,12,2,8,0,1,4]}[pos]       # 윤곽 정렬 위치
            inli = {5:[1,2,6,10,9,8,4,0], 6:[2,3,7,11,10,9,5,1],
            ...
            # 순서 맞추기
            # -> 정렬 값 에서 12시 팩을 기준 시계방향 순서로 정렬
            # -> 팩 순서 교체 위치에 홀 오는거 해결
            # -> 고장이야 고장 고처라 고쳐
            ...
            for pack in leafli:
                ...
                fix.append(pos1)        # 홀에 넣었던거 고정
                m = bfs(1,m,leaf,pos2,bpack)        # 순서 교체
                fix.append(pos2)

                fix.remove(pos1)        # 홀에 넣었던거 빼기
                m = bfs(1,m,leaf,pos3,pack)
                fix.remove(pos2)

##                print('pos1, pos2, pos3 ->',pos1,pos2,pos3)
##                print('fix ->',fix)
##                print('pack ->',pack)
##                print('behind pack ->',bpack)
##                print('curli ->',curli)
##                print('leafli ->',leafli)
##                printf(m,4,4)
##                printf(leaf,4,4)
...
##    t,r,l,a = 2,'51x73006208x0x04','06x04000513x8x27',[5, 6]
    ...
    # DART(최악 트리플) 연산 2m 24s | 무빙 12m 17s
...
##    t,r,l,a = 2,'380x0x56720100x4','078x3x10450200x6',[13, 12]
    ...
    # TEST
##    t,r,l,a = 2,'06502x701048x3x0','05817x306002x4x0',[9, 7]
    t,r,l,a = 2,'x3x100680020457x','x7x280050040613x',[8, 11]         # many time
    t,r,l,a = 2,'x47360x100258x00','x84702x503160x00',[8, 14]       # ???
