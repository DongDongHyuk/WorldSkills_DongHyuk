...
def main(g_t,m,*a):    
    ...
    if t == 2:
        ...
        if pos:
            ...
            pos1, = [i for i,j in exp(0,m,pos0) if i not in hli]

            # 순서 맞추기 전 셋업
            m = bfs(1,m,leaf,pos0,'0')
            fix.append(pos0)
            
            leafli = [leaf[i] for i in li0 if i not in hold and leaf[i] != '0']
            for pack in leafli:
                curli = [m[i] for i in li0 if m[i] in leafli]
                ...
                m = bfs(1,m,leaf,pos1,pack)                                
                m,step = exc(m,pos0,pos1)
                res.append(step)

                n = li0.index(pos1)-1
                m = bfs(1,m,leaf,li0[n % 8],bpack)
                fix.append(li0[n % 8])
