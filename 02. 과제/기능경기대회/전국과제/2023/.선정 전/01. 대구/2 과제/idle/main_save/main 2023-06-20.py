...
def exc(m,s,e,gli,g=-1,p=-1):
    ...
    if g != -1:                                                 # 점, 선 정렬 - 팩을 잡거나 놓기
        ...
        return [(''.join(m),tuple(gli)),[s,g,s2i(p),e]]         # [팩 놓을 위치,그리퍼,팩,타고 내려올 위치]
    m[s],m[e] = m[e],'0'
...
def exp(n,m,pos=-1):
    ...
    for i in range(size*sz) if n else [pos]:        
        if t == 0 and m[i] not in '0b' and len(gli) < 2 and \
        ...
        if ((n and m[i] != '0') and t != 1) or i in fix:                                    # i가 빈칸이 아닐때(선 정렬 제외 &-0), i가 고정일때
            continue        
        for j in [j for j in aro(i)
            ...
            if t == 1:
                d = abs(i-j)
                if d == 1 and m[j] == 'b' and m[i] not in '0b' and len(gli) < 1:            # 선 정렬 - 팩 잡기 
                    res.append(exc(m,i,-1,gli,1,m[i]))                    
                if m[i] != '0':                                                             # 선 정렬 - i가 빈칸이 아닐때 &-0
                    continue
...
def main(g_t,m,leaf=-1):    
    ...
    t = g_t
    sy,sx,sz = [[1,2,3],[1,7,2],[4,4,1],[3,3,3]][t]
    size = sy * sx
    fix = []
    res = []
    cache = {}      # cache reset
    ...
    if t == 1:
        ...
        for i in range(sx):
            ...
            fix.append(i)            
    if t == 2:
        li = [0,15,3,12,1,7,14,8,2,11,13,4]
        for i in li:
            if leaf[i] != '0':
                m = sort(m,leaf,i)
                fix.append(i)
...
##    Type,root,leaf = 1,'bb43210'+'0000000',-1
    Type,root,leaf = 2,'1056034000780092','1056034000780092'[::-1]
