...
def main(g_t,m,*a):    
    ...
    if t is 1:
        ...
        li1 = list(set(hli[1:]))
        for i in range(3):
            a,b,c,d = [j + i for j in [0,5,10,15]]
            A,B,C,D = [j + i + 1 for j in [0,5,10,15]]
            if C in li1:
                if D in li1:
                   li += [d,c]
                if B in li1:
                ...
                li += [d]
            li += [j for j in [a,b,c,d] if j not in li]
        ...
        for pos in li[:8]:
            ...
            if m[pos] is pack:        # 이미 정렬 되어있을때
                print('이미 {}에 {}팩이 있음'.format(pos,pack))        # PRINT
                printf(m,4,5)        # PRINT
                fix.append(pos)
                continue
            ...
            road = [i for i in road if i not in hli]
            fix.remove(h)
            l = len(road)
            if l > 3:      # 길이 4칸 이상일때 
                n = [2,3,3,4,4][l - 4]
            ...
            for r in road:
                while 1:
                    ...
                    if not [i for i in r if m[i] is not '0']:
                        print('길 다비운 상태 | 팩 : {}, 길 : {}, 최종 정렬 위치 : {}'.
                              format(pack,r,pos))        # PRINT
                        printf(m,4,5)        # PRINT
                        break

                    if s not in fix:        # 옮길 팩이 고정되지 않았을때
                        # 길이 모두 빈칸이거나, 홀이 아닌 곳에서 길찾기
                        pos1 = [i for i in r if m[i] is not '0']
                        pos1 = pos1[0] if pos1 else False
                        if pos1:
                            li1 = list(bfs(-2,m,pos1).keys())       # 고정전
                            fix += [s]       # 고정컨
                            li2 = list(bfs(-2,m,pos1).keys())       # 고정후

                        print('길안에 길찾기(고정 X) :',li1)
                        print('길안에 길찾기(고정 O) :',li2)

                        # 막혔을때 조건
                        # 고정전과 고정후의 길찾기 결과를 비교
                        if pos1 and any([i not in li2 for i in li1 if i is not s]):
                            fix.remove(s)
                    ...
                    print('정렬 위치 : {}\n'
                          ...
                          '길 : {}\n'
                          'fixed : {}\n'
                          .format(pos,m.index(pack),m.index(pack) in fix,pack,r,fix))        # PRINT
                    printf(m,4,5)        # PRINT
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'09007a200b156483c000','a500c840072009b63010',[2, 3, 7]
