...
##                for i in hli[1:]:
##                    fix.append
##                for pos1 in [i for i in range(size) if m[i] != '0' and i not in fix]:
##                    li1 = [i for i in range(size) if i not in fix + hli]
##                    li2 = [i for i in bfs(-1,m,pos1,-1,p = m[pos1]) if i not in hli]
##                    for pos2 in li1:
##                        if pos2 not in li2:
##                            b = True
...
def main(g_t,m,*a):    
    ...
    if t == 1:
        ...
        unhold = []         # 정렬 위치 경로
        li = []         # 정렬 순서
        for li1 in [[0,5,10,15],[1,6,11,16],[2,7,12,17]]:
            li += getli(li1,1)
