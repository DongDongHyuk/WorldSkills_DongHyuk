            ...
            for r in road:
                while 1:
                    ...
                    if not [i for i in r if m[i] is not '0']:
                        print('길 다비운 상태 | 팩 : {}, 길 : {}'.format(pack,r))
                        printf(m,4,5)
                        break

                    if s not in fix:        # 옮길 팩이 고정되지 않았을때
                        fix += [s]       # 고정컨
                        # 길이 모두 홀이거나, 홀이 아닌 곳에서 길찾기
                        pos1 = [i for i in r if i not in hli]
                        pos1 = pos1[0] if pos1 else False
                        print('길에 포함된',pos1,'에서 막혔는지 확인',end = '')
                        if pos1:
                            li1 = list(bfs(-2,m,pos1).keys())
                        print('결과 :',li1)

                        # 길이 모두 홀 -> 길이 뚫려 있음
                        
                        if pos1 and len(li1) <= len(r):
                            fix.remove(s)
                    ...
                    print('정렬 위치 : {}\n'
                          ...
                          '길 : {}\n'
                          'fixed : {}\n'
                          .format(pos,m.index(pack),m.index(pack) in fix,pack,r,fix))
                    printf(m,4,5)
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'0010026089ba450073c0','00390410700a0c0b8562',[0, 7, 4]
