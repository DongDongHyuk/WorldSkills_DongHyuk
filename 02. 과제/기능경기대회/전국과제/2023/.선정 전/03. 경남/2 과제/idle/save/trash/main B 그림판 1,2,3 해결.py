...
def main(g_t,m,*a):    
    ...
    if t is 1:
        ...
        def getli(li,n):
            li1 = []
            li2 = hli[1:]+fix       # color hole + fix
            for i in [1,2]:
                if all([j+n in li2 for j in li[i:]]):
                    li1 += [j for j in li[i:]][::-1]
                    break
            else:
                for i in li:
                    if i+n in li2:
        ...
        for ct,pos in enumerate(sli):
            ...
            if ct < 4:      # 고정 고정 쇼
                for i in sli1:
                    if i in sli[:ct] and i not in hli:
                        continue
                    if i in hli:
                        # 고정고정 할때 색상 홀이 두개 가로로 붙어서 중앙에 걸쳐있을때 컨티뉴
                        if abs(hli[1] - hli[2]) is 1 and \
                    ...
                    ffli.append(i)
            if ct < 8:      # 바깥쪽 줄 한개씩 뿅
                #[그림판 1번, 2번]중앙(7, 12) 색상 홀 갯수
                n1 = len([i for i in hli[1:] if i in [7,12] ])
                # [그림판 1번]두 번째 줄 정렬, 중앙에 색상홀, 색상홀이 가로로 나란히
                b1 = (ct > 3 and n1 and abs(hli[1] - hli[2]) is 1)
                if b1:
                    b2 = y is 3
            ...
            if not b1:      # [그림판 1번] 아닐 때
                # 첫 번째 줄 첫 정렬, [그림판 3번](두 번째 줄 첫 정렬, 고정 위치 덜 고정)
                if not ct or (ct is 4 and n < 4):
                    li = [0,1,2,3,4] if y is 3 else [15,16,17,18,19]
                    li = [i for i in li if i not in fix]
                    m = bfs(2,m,li,[],-1)
                    fix += li
                # [그림판 2번]두번째 줄 정렬, ((중앙 색상홀 한개, 고정위치 다 고정), 중앙 색상홀 두개)
                if ct > 3 and ((n1 is 1 and n is 4) or n1 is 2):
                    pos1 = [5,10] if b else [9,14]
                    pos1 = [i for i in pos1 if i not in hli][0]
                    fix.remove(pos1)
         
            print(pos,'에',pack,'팩 정렬')
            print('fixed :',fix)
            printf(m,4,5)
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'851030004a0b07006c29','9027608040c000b051a3',[15, 7, 12]
