...
def exp(n,m,pos=-1):
    ...
    for i in range(size) if n > 0 else [pos]:       
        ...
        for j in aro(i):
            ...
            b2 = lambda hp,pos: (hp is hli0[0] or 
                                (hp is hli0[1] and m[k] in '123456') or
                                (hp is hli0[2] and m[k] in '789abc'))
            if not b1(j):
                continue
            if t < 2 and n > 0:
                if j in hli0:
                    ...
                    for hp in li:           # hole position
                        ...
                        for k in aro(hp):
                            # t is 1 - 팩 별로 지나갈 수 있는 홀 조건(검 빨 파)
                            if b1(k) and k not in li and \
                               (not t is 1 or b2(hp,k)):
                                if m[k] is '0':
                                    if k in hli0:
                                        li.append(k)
                                    continue
        ...
        if n is 1:
            if (pos is -1 and cur == leaf) or \
               (pos is not -1 and cur[pos] is pack):
                break
        if n is 2 and cur[pos] not in ['0']+li:
            break
...
def sort(m,leaf,e,p=-1):
    ...
    if t is 1:   
        li = [i for i,j in exp(0,m,e) if not ((i is hli[1] and pack in '789abc') or
                                              (i is hli[2] and pack in '123456'))]    
        b = m[e] is not pack and len(li) is 1 # 빈칸 고정 박고 움직일건지
        
        hp = hli[2 if pack in '123456' else 1]
        fix.append(hp)      # 지날 수 없는 홀 고정 컨
        r = [s]+bfs(0,m,s,e)
        fix.remove(hp)
...
##        printf(leaf,4,5)
        ...
        pos = [0,1,15,16]
        pos = [i for i in pos if i not in hli][0]
        n = pos if leaf[pos] is not '0' else bfs(-1,m,leaf,pos,[])[-1]
        pack = leaf[n]

        li = [[4,9,14,19],[15,16,17,18]]

        fix += li[0] if m.index(pack) in li[1] else li[1]
        
        m = sort(m,leaf,pos,pack)         # 홀 일때 어캐할꺼임
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'07001a8300906500b4c2','0103080ca900270b0645',[14, 11, 0]
