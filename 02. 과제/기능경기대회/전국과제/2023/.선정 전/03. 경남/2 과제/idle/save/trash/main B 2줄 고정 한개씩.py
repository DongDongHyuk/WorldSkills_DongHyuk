...
def main(g_t,m,*a):    
    ...
    if t is 1:
        ...
        fli = [i for i in fli if i not in hli]
...
##                print(sli,'- [{}] |'.format(pos),'Skip - already sorted\n')
            ...
            for i in fli:       # 정렬 할때 고정
                # 2줄 고정할때 안쪽줄할때는 고정고정한거 중에 한칸 옆에 한거 고정 풀기
                if i in [1,6,11,16,3,8,13,18]:
                    for j in [j for j in ffli if j not in sli]:
                        fix.remove(j)
                        ffli.remove(j)
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    ...
    Type,m1,m2,a = 1,'30564ac01007000908b2','0054c2000730086ba091',[1, 7, 12]
