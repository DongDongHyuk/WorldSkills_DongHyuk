    ...
    if t is 1:
        ...
        for ct,pos in enumerate(sli):
            ...
            for i in sli:       # 고정고정
                ...
                fix.append(i)
                ffli.append(i)
            for i in fli1:      # 바깥쪽 줄 한개씩 뿅
##                print('{} 에 {} 정렬 할거임'.format(pos,pack),'| 현재 고정할 위치',i)
                m = bfs(2,m,[i],[],-1)
                m = bfs(2,m,[],[i],pack)
                fix.append(i)                
##                printf(m,4,5)
            for i in [i for i in ffli if i not in sli]:         # 안쪽줄 고정 할때 한칸 옆에 고정고정한거 고정 풀기
                fix.remove(i)
                ffli.remove(i)
            for i in range(1,len(fli2)+1):      # 안쪽 줄은 한번에 뿅
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'00102c9b7005a6380400','90b0070400ac10328506',[1, 9, 3]
