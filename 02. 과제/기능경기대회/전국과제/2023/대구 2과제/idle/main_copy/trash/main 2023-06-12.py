...
def exc(m,s,e,gli,g=-1,p=-1):
    ...
    if t < 2:
        gli = list(gli)
    if e == -1:                     # 점, 선 정렬 - 팩을 잡거나 놓기
        m[s] = '0' if g else p
        gli.append(p) if g else gli.remove(p)
        return ((''.join(m),tuple(gli)),[s,g,p])    
    m[s],m[e] = m[e],'0'
    if t == 1 and m[s] == 'b':      # 선 정렬 - 다리 이동
        ...
        if n:
            ...
            m[n+(s-e)],m[n] = m[n],'0'
        if m[s+sx] != '0':
            return ((''.join(m),tuple(gli)),[e+(e-s),e])
    return ((''.join(m),tuple(gli)) if t < 2 else ''.join(m),[e,s])
    ...
    if t == 3:
        for i in range(size*(sz-1)):
            if leaf[i] != '0':
                m = sort(m,leaf,i)                
        bfs(1,m,leaf,-1,-1)      
    print('before',aro.cache_info())
    aro.cache_clear()       # cache reset
    print('after',aro.cache_info())
    return res

if __name__ == '__main__':
    
    Type,root,leaf = 0,'514203',-1
