...
def main(g_t,m,*a):    
    ...
    if t is 1:
        ...
        if not b:
            ...
            fli1 = getli([4,9,14,19],-1)        # 고정 순서 1
            fli2 = getli([3,8,13,18],-1)        # 고정 순서 2
        else:
            sli = getli([4,9,14,19],-1)         # 정렬 순서
            fli1 = getli([0,5,10,15],1)         # 고정 순서 1
            fli2 = getli([1,6,11,16],1)         # 고정 순서 2
        fli1 = [i for i in fli1 if i not in hli]
        fli2 = [i for i in fli2 if i not in hli]
        for ct,pos in enumerate(sli):
            '''
            1. 양쪽한 줄을 먼저 정렬할거임(한개 씩)
            2. 정렬할 줄의 반대 2줄을 아무팩 넣고 고정, 단 정렬할 팩은 안됨
            3. 이때 정렬할 줄에 아무팩 넣고 고정, 이때는 정렬할 팩도 포함되도됨
            '''
            fix = sli[:ct]
            if pos in hli:      # 정렬 위치가 홀이올시다.
##                print(sli,'- [{}] |'.format(pos),'Skip - sort pos is hole pos\n')
                continue
            # 정렬 할 팩
...
##                print(sli,'- [{}] |'.format(pos),'Skip - already sorted\n')
                continue
            
            # 고정쇼
            ffli = []       # 고정고정리스트
            for i in sli:       # 고정고정
                if i in sli[:ct] and i not in hli:      # 정렬된 위치이고 홀이 아닐때
                    continue
                '''
                고정할때 고정위치에 홀이 2개 이상 있어버리면 고정을 2개 밖에 못하거든요??
                그래서 홀위치 한칸 옆에 팩을 고정 박아버릴게요
                '''
                if i in hli:
                    i += -1 if b else 1
                if i in hli:
                ...
                ffli.append(i)
##
##            print('고정하기위해 고정함',fix)
##            printf(m,4,5)

            for i in fli1:
##                print('{} 에 {} 정렬 할거임'.format(pos,pack),'| 현재 고정할 위치',i)
                m = bfs(2,m,[i],[],-1)
                m = bfs(2,m,[],[i],pack)
                fix.append(i)                
##                printf(m,4,5)

            # 2줄 고정할때 안쪽줄할때는 고정고정한거 중에 한칸 옆에 한거 고정 풀기
            for i in [i for i in ffli if i not in sli]:
                fix.remove(i)
                ffli.remove(i)

            # 2줄 중 안쪽 줄은 한번에 뿅
            for i in range(1,len(fli2)+1):
                li1 = [j for j in range(size) if j not in fix and m[j] not in ['0',pack]]
                if i > len(li1):
                    i -= 1
                    break
            ...
            # 진짜 마지막 고정쇼
            '''
            홀위치에 따라서 마지막 고정쇼를 어캐할지 바꿔야 될듯
            -> 첫 정렬에만 해볼게요
            만약 이거 안되면 정렬 위치에 홀이 아예 없을때 ㄱㄱ
            '''
            if not ct:
                y,x = divmod(pos,5)
...
##            print(sli,'- [{}] |'.format(pos),pos,'에',pack,'팩 정렬할게요')
##            print('hli :',hli)
##            print('fixed :',fix)
##            printf(m,4,5)
            
            m = bfs(1,m,leaf,pos,pack)        # default
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'bac93000016000508247','0205b4000060ca879103',[11, 6, 7]       # 1.701
    Type,m1,m2,a = 1,'758b0020361000ca0094','020a590048300071c06b',[7, 17, 12]
