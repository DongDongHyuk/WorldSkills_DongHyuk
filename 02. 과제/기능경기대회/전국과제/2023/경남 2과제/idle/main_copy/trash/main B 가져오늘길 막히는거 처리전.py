...
##                print('멀어서 도착지에 가깝게 이동 {} -> {}'.format(s,pos1))
            ...
            for i in r[::-1]:
                ...
                if i in hli:        # 홀은 원래 뚫려 있지롱
                    continue

                if i not in st:         # 아직 안 뚫은 곳 일때
                    li = [j for j,k in exp(0,m,i,m[i])]         # 스텝 제외
                    li = [j for j in li if j not in st]         # 뚫었던 길중 막힌 곳 제외
                    li1 = []        # 제외 목록
                    for j in li:
                        b1,b2 = m[j] != '0',j in hli
                        if b1:         # 간접 적으로 막힘 제외
                            li2 = [k for k,l in exp(0,m,j,m[j]) if k != i]                            
                        if b2:         # 홀이 있는데 홀을 타고 갈곳이 없음 제외
                            li2 = [k for k,l in exp(0,m,j,m[i]) if k != i]
                        if b1 or b2:
                            if not li2:
                                li1.append(j)                            
                    li = [j for j in li if j not in li1]
...
##                print(road,'DDOING')
                ...
                if i not in st:
                    if b or (len(li) == 1 and li[0] in r):         # 현재 뚫은 위치가 막힌 곳있가
                        st.append(i)
            if s in fix:
                break
    ...
    if t == 1:
        ...
        for pos in li:            
            if pos in hli:      # 정렬 위치가 홀
                continue            
##            if len([i for i in fix if m[i] != '0']) == 12:      # 떙겨올 팩이 없음
##                break
            # 정렬할 팩
...
##    t,m1,m2,a = 1,'04053706100280a0c90b','6a05890c004100b27300',[18, 9, 6]          # 간접적으로 막힘
    ...
    t,m1,m2,a = 1,'040c0a25830906b07010','6043a28010500b00c970',[15, 12, 19]        # 강제 회전초밥
