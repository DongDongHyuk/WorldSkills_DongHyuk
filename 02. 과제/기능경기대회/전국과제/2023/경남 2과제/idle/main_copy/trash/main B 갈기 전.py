...
def bfs(n,m,*a):
    ...
    while 1:
        ...
        if n is -1:
            ...
            pack = m0[cur]
            if pack is not '0' and cur not in li:
                fc = fix[:]
                ...
                y1,x1 = divmod(s,5)
                y2,x2 = divmod(cur,5)
                if abs(y1 - y2) + abs(x1 - x2) is len(r):
                    break
        ...
        if n is 2:
            if not [i for i in li1 if cur[i] is not '0'] and \
               ...
               pack not in [cur[i] for i in li2]:
                break
        if n is 3 and all([cur[i] is leaf[i] for i in li]):
            break
...
def main(g_t,m,*a):    
    ...
    if t is 1:
        ...
        holdct = []
        hold = {}
        li = [0,1,2,5,6,7,10,11,12,15,16,17]        # 고정 위치를 제외한 곳
        b = len([i for i in li if i in hli[1:]]) >= 2
        x1,x2,x3,x4,x5 = [[j * 5 + i for j in range(4)] for i in range(5)]
        # 정렬 순서 1~3, 고정 순서 1~2
        if not b:
            li1,li2 = [x1,x2,x3,x5,x4],[1,1,1,-1,-1]
        else:
            li1,li2 = [x5,x4,x3,x1,x2],[-1,-1,-1,1,1]
        sli1,sli2,sli3,fli1,fli2 = [getli(i,j) for i,j in zip(li1,li2)]        
        fli1 = [i for i in fli1 if i not in hli]
        fli2 = [i for i in fli2 if i not in hli] 
        sli = sli1+sli2+sli3
        for ct,pos in enumerate(sli):
            fix = sli[:ct]
            y,x = divmod(pos,5)
            if pos in hli:      # 정렬 위치가 홀이올시다.
                continue
            # 정렬 할 팩
            ...
            if ct < 4:      # 고정고정
                for i in sli1:
                    if i in sli[:ct] and i not in hli:
                        continue
                    if i in hli:
                        # 원래 위치 한칸 옆에 고정고정 할때 색상 홀이 나란히 중앙에 걸쳐있을때 컨티뉴
                        if abs(hli[1] - hli[2]) in [1,5] and \
                           any([j in [2,7,12,17] for j in hli[1:]]):
                            continue
                        i += -1 if b else 1
                    if i in hli:
                        continue
                    ...
                    fix.append(i)
                    ffli.append(i)
            if ct < 8:      # 바깥쪽 줄 한개씩 고정
                n1 = len([i for i in hli[1:] if i in [7,12] ])      #중앙(7, 12) 색상 홀 갯수                
                b1 = ct > 3 and ((n1 and abs(hli[1] - hli[2]) is 1) or n1 is 2)         # [그림판 1번,2번]
                if b1:
                    b2 = y in [2,3]
                    li = [0,1,2,3,4] if b2 else [15,16,17,18,19]
                    li = li[::-1] if b else li
                    li = [i for i in li if i not in fix+hli]
                for i in li if b1  else fli1:
                    if m[i] is pack:
                        m = bfs(2,m,[i],[],-1)
                    m = bfs(2,m,[],[i],pack)
                    fix.append(i)
            if ct < 4:      # 안쪽줄 고정 할때 한칸 옆에 고정고정한거 고정 풀기
                for i in [i for i in ffli if i not in sli1]:
                    fix.remove(i)
                    ffli.remove(i)
                for i in range(1,len(fli2)+1):      # 안쪽 줄은 한번에 고정
                    li = [j for j in range(size) if j not in fix and m[j] not in ['0',pack]]
                    if i > len(li):
                        i -= 1
                        break
                    m = bfs(2,m,[],fli2[:i],pack)
                fix += fli2[:i]
                for i in ffli:      # 고정고정했던거 풀기
                    fix.remove(i)                    
            # 진짜 마지막 고정
            n = len(fli1)       # 고정 위치 고정 갯수
            if not b1 and ct < 8:      # [그림판 1번],[그림판 2번] 아닐 때
                if not ct or (ct is 4 and n < 4):       # 첫 정렬, [그림판 3번]
                    li = [0,1,2,3,4] if y is 3 else [15,16,17,18,19]
                    li = [i for i in li if i not in fix]
                    m = bfs(2,m,li,[],-1)
                    fix += li
                if ct > 3 and n1 is 1:
                    if n is 4:       # [그림판 4번]
                        pos1 = [5,10] if b else [9,14]
                        pos1 = [i for i in pos1 if i not in hli][0]
                        fix.remove(pos1)
                    if ct > 4 and n is 2:      # [그림판 5번] 
                        li = [1,2,3] if y in [2,3] else [16,17,18]
                        li = li[::-1] if b else li
                        li = li[:-1]
                        for i in range(1,3):
                            m = bfs(2,m,[],li[:i],pack)
...
##        fix = [i for i in sli if i not in hli]
##        li = []
##        for i in sli3:
##            if i not in hli:
##                li.append(i)
...
##    Type,m1,m2,a = 0,'051002426053060','510642030314000',[[0, 9, 14], [2, 6, 12]]
    Type,m1,m2,a = 1,'c0080b016402a0039075','0c0307019b0a84005260',[6, 10, 14]
    Type,m1,m2,a = 1,'00c400b320056a907180','06ba005004128700039c',[5, 15, 4]
