    ...
    if t is 1:
        ...
        while 1:
            ...
            for pos in r[::-1]:               
                ...
                # 치웠던 곳은 팩이 없어서 현재 뚫을 위치에 존재하는 팩으로 길찾기
                # but 다음 치울 곳들은 그곳에 있는 팩도 고려 하셔야죠 ^^??
                
                for pos1 in r[::-1]:
                    b1 = not (pos1 == pos or pos1 in road) and m[pos1] != '0'
                    pack2 = m[pos1] if b1 else pack1
                    li = [i for i,j in exp(0,m,pos1,pack2) if i not in r]
                    li1 = []        # 갈 수 없는 홀
                    for i in li:
                        
                        if m[i] != '0':         # 갈 수 없는 위치 기르기
                            while 1:
                                cur = i
                                li1 = [j for j,k in exp(0,m,cur,m[cur]) if j != pos1]                                    
                            
                        if i in hli:        # 갈 수 없는 홀 거르기
                            if not [j for j,k in exp(0,m,i,pack2) if j != pos1]:
                                li1.append(i)
...
##    t,m1,m2,a = 1,'a20814050976000b003c','b492000007a500680c13',[13, 16, 6]         # 간접적으로 막힘(신)
    ...
    t,m1,m2,a = 1,'9040a506802007bc1003','024000080a59017630cb',[12, 6, 17]
