...
def con(m):
    m = [('abc'[i-10] if T == 1 and i < 20 else '0') if i>9 else str(i) for i in m]
    return ''.join(m)
for i in range(3):
    T = i
    m1 = root[i]
...
def ps(pos,y,x,sy=50,sx=50,err={}):
    ...
    return pos
elcA=[ps(None,3,5,sx=40),ps(None,3,5,sx=40)]
elcB=ps(None,4,5)
elcC=ps(None,3,4)
airA=[ps(None,3,5,sx=40),ps(None,3,5,sx=40)]
airC=ps(None,3,4)
poss=[[elcA,elcB,elcC],[airA,None,airC]]

def mt1(p,g,d=[],mod=0,h=-1,z=0,a1=3000,a2=1000,b1=1,b2=0):
    tp = tr((pos[ing] if T == 0 else pos)[p],[0,0,-z,0,0,0])
    ml(up(tp,mod,h),a=a1)
    if b2:
        return -1
    ml(tp,a=a2,r = 5)
...
def Run():
    def getz(pack):
        if not tool:
            return 0
        return 20 if pack in [1,4] else 10 if pack in [2,5] else 0 
    if T == 0:
        global ing
        ing = 0
    if T == 1:
    ...
    for i in range(len(res)):
        ...
        if T != 1 and len(r) == 4:
            for j in range(4):
                ...
                mt1(s,1 if j < 2 else 0,[s,pack],0,300,z,2000,1500,0)
                wt(0.1)
                ml(up((pos[n] if T == 0 else pos)[s],0,300))
                if not n:
                    cht()
            continue
        b3 = len(r[0]) > 2
        ...
        b4 = T == 2 and s in info[2][0]
        if T == 0:
            ing = r[-1]
        if not b2:
            z = getz(pack)
            mt1(s,1,[s,[0,pack+10][b4]],z=z,b1=0)
            rml(z=3,acc=500)
        if b3:
            for i in road:
                mt1(i,-1,[],1,3,z,1500,b2=1)
        mt1(e,0,[e,pack],1,3,z,[None,1500][b1],500,b2=b1)
