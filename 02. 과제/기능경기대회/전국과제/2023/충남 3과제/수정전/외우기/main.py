...
def mt1(p,g,d=[],mod=0,h=-1,z=0,a1=2000,a2=1000,b1=1,bz=0):
...
def A():
    ...
    for i in range(4):
        e =  main(0,m,s,-1,li)
        key = li.index(e)+1
        road2 = main(0,m,s,e,-1)
        pos1 = read(20,2,2).index(10+key)
        mt1(elcA1[pos1],1,[20+pos1,0])
        ml(up(elcA[s]))
        for j in road2[:-1]:
            ml(up(elcA[j],1,5))
        mt1(e,0,[e,10+key],1,5,a2 = 500,b1=0)
        rml(z=70)
        pos2 = m.index(str(key))
        mt1(pos2,1,[pos2,0])
        mt1(elcA1[pos1],0,[20+pos1,key])
        li[key-1] = -1
        m = ''.join(['x' if j == e else '0' if j == pos2 else m[j] for j in range(20)])
    s,e = info[T][1]
