...
def exc(m,s,e):
    if t == 1:
        ...
        pack = m[s] if g else e             # pack
        m[s] = '0' if g else e              # 맵 수정
        n = gli.index('0' if g else pack)
        gli[n] = pack if g else '0'         # 그립 리스트 수정
        info = [s,int(pack),g,n]            # 위치, 팩, 그리퍼상태, 그리퍼종류
        ct = gli.count('0')
...
def exp(n,m,pos=-1):
    ...
    for i in range(9) if n > 0 else [pos]:
        ...
        if n > 0 and m[i] != '0' and t != 2:
            continue        
        li = [aro(i), None, range(9), None][t]
        for j in [j for j in li if m[j] != '0' and j not in fix]:
            if t == 0:
                pack = m[j]
...
def bfs(n,m,*a):
    ...
    while 1:
        ...
        if n == 0 and cur == e:
            break
        if n == 1:
            cur1 = cur[0] if t in [1,2] else cur
...
if __name__ == '__main__':
    ...
    t,m1,m2 = 0,'021345670','307541620'         # 예시 배치
    ...
    t,m1,m2 = 1,'726540310','146053072'         # 예시 배치    
    ...
    t,m1,m2,a = 2,'615403702','234567100',0         # 예시 배치
