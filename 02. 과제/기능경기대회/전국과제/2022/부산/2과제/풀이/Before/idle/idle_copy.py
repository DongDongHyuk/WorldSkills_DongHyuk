...
def main(r:'root',l:'leaf',T: 'Types of Parets'):
      #Global Variables
      global root,leaf,Type,fix,wall,val,p_li # 고정,벽,오차
      ...
      if Type == 0:
            ...
            def Scoring(start,end):
                  ...
                  return score
            Sequence = (0,5,10,15,16,11,6,1,2,3,4)# 정렬 순서
            Sequence_p = [leaf[i] for i in Sequence]
            if Sequence_p.count('h') == 0: Sequence = (19,14,9,4,3,8,13,18,17,16,15)
            marked = {'leaf':root}
            for idx in Sequence:
                  if leaf[idx] == 'h':
                        fix[1].add(idx) # 수동 고정
      ...
      if Type == 1:
            fix[1].add(root.index('c')) # '차' 고정
            marked = bfs(root,1) # 궁 주변에 빈칸이 1개 일때   
            result += path(marked)           
            fix[1].update([i for i in p_li if marked['leaf'][i] != '0']) # 궁 주변 팩 모두 고정
            fix[1].remove(root.index('c'))
...
#root = '0ab00c9700d8h5h62431'
#leaf = '005160003d9ah4h8b72c'

#main(root,leaf,0) # Runing Main.

root = 'm000c0s000p0gsmc000p'
#root = '0000p00000pg0mcss0cm'
leaf = None
