def printl(a,string = "none",count = 1):
    for i in range(24):
        ...
        if len(str(a[i])) == 1 and count % 4 != 0: print(" ",end = "") 
        count += 1
    if string == "": print("")
...
def find_road(li,start = [],arrival = [],count = 1):
    B = li[:] # 리스트 저장
...
ex_list = [2,2,1,1,0,0,9,0,0,0,0,9,0,9,0,0,0,0,9,0,1,2,2,1] #파레트
printl(find_road(ex_list,[1,1],[6,4]))
