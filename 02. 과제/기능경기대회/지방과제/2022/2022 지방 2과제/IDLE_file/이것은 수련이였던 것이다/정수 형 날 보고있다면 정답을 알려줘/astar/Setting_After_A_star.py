...
def seting_br():
    ...
    global line
    numpos = br.index(3)
    if numpos in [0,1,2]: line = [0,1,2]
    elif numpos in [5,8]: line = [2,5,8]
    elif numpos in [6,7]: line = [6,7,8]
    elif numpos in [3]: line = [0,3,6]
    line.remove(numpos)
    while True:
        if br[line[0]] == goal_br[line[0]] and br[line[1]] == goal_br[line[1]]:break 
        recoding_node() # 노드 기록
        comparing_cases(1) # 경로 
        comparing_node() # 노드 선택
        #printl(br,"")
        count += 1
    print("complete setting !!! step: {}".format(count));print("")
    printl(br,"")
