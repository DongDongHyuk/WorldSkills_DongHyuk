...
#selecting_node            
def comparing_node():
    global br,br_box,recode_br
    for i in range(len(br_box)): # 다음 보드 후보 
        if br_box[i] not in recode_br: # 이 후보는 기록에 없으면
            br = br_box[i] # 다음 보드 확정
            br_box.clear() # 이제 필요없어
            break
...
def random(print_ = "off", count = 0):
    ...
    while goal_br != br:
        ...
        comparing_node()
        recoding_node()
        if print_ == "on": printl(br,"")
        count += 1
    if goal_br == br: # 완성 하셨나요?
        print("complete !!! step: {}".format(count)); print("")
        print("goal_br"); printl(goal_br,"")
        print("br"); printl(br,"")
        optimizating("on")
