...
sl = lambda pos,abc = [0,0,0]:trans(pos,[0,0,0]+abc)
us = lambda pos,abc,h=None: up(sl(pos,abc),h)

pos = [posx(-180.07, 282.03, 16.21, 121.69, -180, 31.42)] * 25
for i in range(1,25):
    if i % 5:
        pos[i] = trans(pos[i-1],[-40,0,0,0,0,0])
    else:
        pos[i] = trans(pos[i-5],[0,40,0,0,0,0])
posB = [posx(338.51, 350.37, 5.81, 90, 115, 89.73),
            posx(338.51-40, 350.37-10, 5.81, 90, 115, 89.73),
            posx(338.52-80, 350.37-20, 5.81, 90, 115, 89.73)]
