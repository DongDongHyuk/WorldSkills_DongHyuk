...
def bfs(start: str, goal: str,MAX: int):
    que = deque()
    que.append(start) # 첫 번째 노드 
    marked = {start: "start"} # 마킹
    board = 'str'
    while board != goal:
        if len(marked) > MAX: return "MAX ERR"
        board = que.popleft()
...
def main():
    ...
    MAX = 180000# max_value
    marked = bfs(start_state,goal_state,MAX)
    if (marked == "MAX ERR"):
        return print("Maximum Exceeded (visit_node {} ~ ∞ )".format(MAX))
